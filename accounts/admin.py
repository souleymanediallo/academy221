...
class CustomUserAdmin(admin.ModelAdmin):
    fieldsets = (
        (None, {'fields': ('email', 'password', 'first_name', 'last_name', 'email_confirmed')}),
        ('Permissions', {'fields': ('is_admin', 'is_staff', 'is_superuser', 'is_active')})
    )
    list_display = ['first_name', 'last_name', 'email', 'email_confirmed', 'date_joined', 'last_login']
...
class ProfileAdmin(admin.ModelAdmin):
    ...
    def user_info(self, object):
        return f'{object.user.last_login}'
